...
**Key files:**
- `src/Pilotiq.ts` — Builder: `.path()`, `.branding()`, `.theme()`, `.layout('sidebar'|'topbar')`, `.resources()`, `.globals()`, `.pages()`, `.schema()`, `.guard()`, `.user(req => userOrNull)` (Plan #10 — opaque resolver, sync or async, throws → null), `.use(plugin)`. Also exposes `pilotiq.resolveUser(req)` for routes / `panelInfo()`.
- `src/Resource.ts` — abstract class with **static** methods: `label`, `labelSingular`, `slug`, `icon`, `model`, `form(form)`, `table(table)`, `detail(record)`, `deleteRecord(id)`, `pages()`, `resolvePages()`, `relations()`, `getSlug()`. Resources register as classes, not instances. `resolvePages()` overlays user `pages()` over `defaultPages(this)`. `model?: ModelLike` opts the resource into auto-wired CRUD — see ORM section below. **Navigation metadata (Plan #9):** seven optional statics drive sidebar / topbar grouping + sort + sub-nav + count badge — `navigationGroup`, `navigationSort`, `navigationLabel` (sidebar-only label override; titles still use `label`), `navigationIcon` (same `IconValue` contract as `icon`), `navigationBadge` (`() => string|number|undefined|Promise<…>` resolved server-side in parallel; errors swallow), `navigationBadgeColor` (`'default'|'primary'|'success'|'warning'|'destructive'|'info'`), `navigationParentItem` (JS class name of parent — stable across slug edits; dangling refs and cycles render at top level). Plus `recordTitleAttribute` for record references (Plan #12 search). Helpers `getNavigationLabel()` / `getNavigationIcon()` fall through to `label` / `icon`. **Authorization (Plan #10):** six async predicates default to `true` — `canAccess(user)` (coarse "should this resource exist for this user" gate; failing it drops the resource from nav + 403s every route), `canViewAny(user)` (index/list page), `canView(user, record)` (view page), `canCreate(user)` (create page + auto-hides `Action.create` factory), `canEdit(user, record)` (edit page + auto-hides `Action.edit` factory), `canDelete(user, record)` (delete + auto-hides `Action.delete` factory). User is opaque (whatever `Pilotiq.user(req => …)` returns). Throwing predicates fail closed.
...
- `src/Global.ts` — abstract class for singleton resources (site settings, brand config). Same shape as `Resource` minus list/create/delete; ships `{ edit }` by default, `view` opt-in via `pages()`. Carries the same nav metadata fields as `Resource` (minus `recordTitleAttribute`); `navigationGroup` defaults to `'Settings'` — pass `null` to opt out. **Authorization (Plan #10) subset:** `canAccess(user)` + `canView(user, record)` + `canEdit(user, record)`. (No list / create / delete — singletons don't have those routes.)
- `src/Page.ts` — page class with `static slug/label/icon`, `static schema(ctx)`, plus `static getResource()` (back-reference to owning Resource, undefined for standalone pages) and `static getMode()` returning `'list'|'create'|'edit'|'view'|'custom'`. Carries the same nav metadata fields as `Resource` (minus `recordTitleAttribute`). **Authorization (Plan #10):** single `canAccess(user)` gate (custom pages are too freeform to assume a record concept; resource-bound default page subclasses still inherit their owning resource's predicates via the route handlers).
- `src/defaultPages.ts` — Owns four exported base classes `ListPage` / `CreatePage` / `EditPage` / `ViewPage`. Each derives slug/label/icon from `getResource()`, calls the resource's `form()`/`table()`/`detail()`, applies model-backed defaults via `applyFormDefaults` / `applyTableDefaults`, and exposes override hooks (`getHeader`, `getActions` on ViewPage). Per-resource subclass usage: `class ListArticles extends ListPage { static override getResource() { return ArticleResource } }`. Factories `defaultListPage(R)` / `defaultPages(R)` etc. return anonymous subclasses bound to R for the no-customization path. **Filament-style explicit actions:** `getHeaderActions` / `getRowActions` (`ListPage`) and `getActions` (`ViewPage`) return `[]` by default — users opt in via override or via `Resource.table().headerActions(...) / .recordActions(...) / .bulkActions(...) / .actions(...)`. All four slots accept `Array<Action | ActionGroup>` — `ActionGroup`s render as labelled dropdown triggers with the same placement stamping as plain Actions. Pre-built factories `Action.create(R, base) / Action.edit(R, base, recordId?) / Action.view(R, base, recordId?) / Action.delete(R, base, recordId?)` produce the standard CRUD shapes; pass `recordId` for view-page contexts (URL baked) or omit for row contexts (renderer fills the `:id` template). **Plan #10 auto-visibility:** each factory auto-attaches a `.visible(({ user, record }) => R.canX(user, record))` rule consulting the matching policy (`canCreate / canEdit / canView / canDelete`). Opt out with explicit `.visible(true)` after the factory. Implementation uses a structural `ResourceLike` shim in `actions/Action.ts` to avoid an Action.ts → Resource.ts import cycle; missing predicates are treated as "always allowed." Sentinel `save`/`loadRecord` only fire when the user hasn't configured them on the Resource form (and `R.model` isn't set). `CreatePage`/`EditPage` also expose the form lifecycle as optional static fields (`beforeCreate`, `afterUpdate`, `handleCreate`/`Update`, `mutateFormDataBeforeFill`/`AfterFill`, `mutateData`/`Before*`, `getRedirectUrl`, `getCreatedNotificationTitle`/`getSavedNotificationTitle`); `installLifecycleHooks` wires them onto the form during `schema()` so page-class overrides layer on top of `Resource.form()` config. Default success toast is `"${R.labelSingular} created/saved"` unless overridden; returning `null` from the title hook suppresses it. **List-page tabs:** `ListPage.getTabs()` returns `ListTab[]` (defaults to `[]`). Non-empty arrays get wrapped in a `ListTabs` container element inserted between the heading and the Table. Each tab is a query shortcut + count badge — see the `Tab.ts` / `pageData.resolveActiveTab` description below.
- `src/Tab.ts` + `src/elements/ListTabs.ts` — Filament-style query-shortcut tabs above a list-page table. `ListTab.make(name).label().icon().badge(s|fn).badgeColor().default().modifyQuery(fn).modifyContext(fn)`. `modifyQuery` is the typed/declarative path used by `modelTableRecords` (chained next to `Filter.query()`); `modifyContext` is an escape hatch for user-supplied `Table.records()` handlers. Render-time setters `withActive` / `withResolvedBadge` / `withUrl` populated by the framework. `ListTabs` is a thin container (not user-facing — `ListPage.schema()` builds it from `getTabs()`). Distinct from the form-side `Tab` (under `Tabs.make().tabs([…])`) which holds form children rather than narrowing a query.
...
- `src/pageData.ts` — Per-page-role data builders (`dashboardData`, `resourceIndexData`, `resourceCreateData`, `resourceEditData`, `resourceViewData`, `globalEditData`, `globalViewData`, `customPageData`) plus `dispatchPageData(pageContext)` switchboard. Each builder calls `pilotiq.resolveUser(req)` once and threads the user into both `panelInfo(pilotiq, req)` and the `SchemaContext` (via `userCtx(ctx, user)`) so action visibility predicates see it during `resolveSchema`. **Navigation tree:** `panelInfo(pilotiq, req?)` is async — it builds a unified `navigation: NavItem[]` (`{ name, label, url, icon?, group?, sort?, badge?, badgeColor?, children? }`) by flattening resources → globals → pages, resolving `navigationParentItem` references into nested children (cycles broken with a console warn), sorting siblings by `navigationSort` then registration order, and resolving every `navigationBadge()` in parallel via `Promise.all` (errors swallow). Replaced the legacy `resources / globals / pages` arrays. **Plan #10 nav filter:** `buildNavigation` pre-evaluates `canAccess(user)` for every resource / global / page in parallel (`safeAccess` swallows throws as `false`); items that fail are dropped from the tree entirely. **Both** the rudder route GET handlers AND the Vike `+data` hook (auto-generated by the Vite plugin) call these so SSR and SPA-nav serve identical data. Without the dual path, SPA nav lands on a blank page because the rudder handler doesn't run for `*.pageContext.json` fetches. Page renderers read from `(ctx.data ?? ctx.viewProps)`; the data hook short-circuits to viewProps on SSR to avoid double schema-builds. `resolveActiveTab(elements, query, currentPath)` walks for `ListTabs`, picks the active tab from `?tab=` (default → `ListTab.default()` → first), stamps active flag + per-tab URL + parallel-resolved badge counts; called inside `resourceIndexData` before `loadTableRecords` so `dispatchTable.findActiveTab` can splice the tab's `modifyQuery` into the records-handler `TableContext` (see `dispatchTable.ts`).
- `src/routes.ts` — `registerPilotiqRoutes()` using `view()`. Each GET handler delegates to a `pageData.ts` builder; POST handlers stay here (form submit, action dispatch). **Plan #10 wiring:** every handler resolves the user once via `pilotiq.resolveUser(req)`, then runs `checkPolicy(() => R.canX(user, ...))` for the route's predicate(s); failure → `forbidden(res, json)` (403 JSON when `Accept: application/json`, 403 HTML otherwise). Record-aware predicates (`canView` / `canEdit` / `canDelete`) load the record via `R.model.find(id)` when `R.model` is set, else pass `{ id }` stub. 403 ≠ 401 — pilotiq does NOT redirect to login on policy fail; `Pilotiq.guard()` is the 401 layer. Routes:
...
- `src/vite.ts` — `pilotiq()` Vite plugin, generates `(pilotiq)/` pages + `+Layout.tsx` + `+Head.tsx`. Also reads `app/Pilotiq/AdminPanel.ts` (override via `pilotiq({ panels })`) at config time using **jiti** (handles `.ts`/`.tsx`), discovers exported `Pilotiq` instances by `getConfig()` duck-typing, and emits `pages/(pilotiq)/_components.ts` — a manifest module that imports those instances + walks their config to populate `componentRegistry: { ClassName: ClassRef }` at module-init. The auto-gen `+Layout.tsx` imports the manifest and passes it to `AppShell`'s `componentRegistry` prop so component-typed icons (`Resource.icon = Newspaper`) resolve at render. Re-runs in `configureServer` on panel-file changes for HMR. Panel files must be import-safe (top-level = builders + class defs; no I/O).
- `src/icons/` — User-extensible icon registry. `registerIcons({ name: Component })` populates a module-level map; `getIcon(name)` looks up at render. Resource/Global/Page `icon` static fields accept either a string (registry key) or a component reference (lucide / tabler / heroicons / phosphor — anything React-shaped). `panelInfo()`'s `serializeIcon(value, ownerClassName)` ships strings as-is and components as `{ class: ownerClassName }`. Client-side `useIconFor()` (in `react/icon-context.tsx`) reads the wire format and resolves: strings → runtime registry, `{class}` → build-time `componentRegistry`. Pilotiq's own chrome (~5 lucide icons inside `SchemaRenderer` + a handful in `ui/*`) keeps direct lucide imports — must work before any user registration. `@pilotiq/pilotiq/icons/lucide` exports a curated `lucideIcons` baseline (~150 icons) users can spread into `registerIcons()`. Component-shape detection accepts both `function` (function components) and `object` (forwardRef / memo) — most icon libs ship forwardRefs.
...
- `src/actions/` — `Action` primitive (single class, `placement: 'inline'|'bulk'|'row'|'header'`, `destructive`, `confirm`, `handler`). Four dispatch modes (mutually exclusive): `Action.href(url)` for link-style (Edit), `Action.method('post'|'put'|'patch'|'delete').action(url)` for form-style (Delete), `Action.handler((ctx) => ...)` for server-dispatched buttons, `Action.submit()` for `<button type="submit">` (default Create/Edit save buttons). Submit actions can target a form they live outside of via `Action.submit().form(formId)` — the rendered button uses HTML's `form=` attribute, which is how `CreatePage`/`EditPage` put Save buttons in the page header without nesting them inside the form. Handler-style actions get a `dispatchUrl` stamped at render time by the route registrar; client POSTs `{ ids?, ...values }` and the handler runs with `ctx.record` / `ctx.records` / `ctx.values` / `ctx.request`. Handler returns `void`, `{ redirect }`, `{ notify }` (single Notification or array — see `src/notifications/`), or any combo; throws → 500.
  ...
  **Conditional visibility**: `.visible(rule)` / `.hidden(rule)` / `.disabled(rule)` / `.authorize(rule)` — rule is `boolean | (ctx) => boolean | Promise<boolean>` with `ActionVisibilityContext { record?, records?, user? }`. `Action.evaluate(ctx)` is async (sync rules still work — auto-promised; throwing rules fail closed → `visible: false`). Non-row placements evaluate at schema-resolve time (`resolveSchema` drops hidden actions, stamps `disabled: true`). **Row-placement actions defer to per-row eval in `loadTableRecords`**, which stamps `_visibleActions: string[]` and `_disabledActions: string[]` on each row (now also threads `user` from `loadTableRecords(elements, query, pathname?, user?)`). Actions with rules emit `conditional: true` on meta so the row renderer knows to consult the lookup. `authorize()` is a semantic alias for `visible()`.
...
- `src/react/AppShell.tsx` — Picks layout mode, renders sidebar or topbar. Wraps the layout in `ToasterProvider` so `useToast()` is available everywhere; reads `notifications` from viewProps and forwards as `initialNotifications`. `panel.navigation` is the pre-grouped tree from `panelInfo()`; `currentPath` flows in from the auto-gen `+Layout.tsx` (Vike's `pageContext.urlPathname`) and drives sidebar/topbar active-link state.
...
- `src/react/layouts/SidebarLayout.tsx` — shadcn Sidebar (collapsible, mobile-responsive). Renders `panel.navigation` as one `<SidebarGroup>` per `group` (with `<SidebarGroupLabel>` when set), nests `children` under each parent via `<SidebarMenuSub>`, and stamps a `<SidebarMenuBadge>` for `badge`/`badgeColor`. Active item is the longest URL prefix of `currentPath` (`/admin/users` won't light up `/admin/user`).
- `src/react/layouts/TopbarLayout.tsx` — horizontal nav variant. Group label becomes a dropdown trigger with members + nested children inside; ungrouped items render as flat link buttons; items with `children` and no group render as their own dropdown trigger that includes the parent's URL as the first menu entry.
- `src/react/ThemeSettingsPage.tsx` — Full theme editor: controls sidebar + live iframe preview
- `src/react/SchemaRenderer.tsx` — Renders resolved schema elements. `TableRenderer` segregates table actions by placement: `header`/`inline` in the top bar, `bulk` in a toolbar that only shows when rows are selected, `row` rendered inline (each Action a small button next to the cell — Filament-style). Wrap row Actions in `ActionGroup` to opt into a dropdown explicitly. Bulk placement triggers a checkbox column (selection state keyed by `row.id` in React state). Heading / description / striped / emptyState chrome ride on `Table.heading() / .description() / .striped() / .emptyState({heading, description, icon})`. **Row click navigation (Filament-style per-cell links):** `Table.recordUrl(r => …)` stamps the URL under `_recordUrl`; each *data cell* wraps its content in a real `<a href>` (`RecordCellLink`) — right-click / cmd-click / middle-click "open in new tab" all work natively, and plain left-clicks SPA-navigate via `useNavigate()` (modified clicks fall through to the browser). Action and bulk-select cells stay unwrapped so clicking a row action fires only the action — no event-bubbling guard, no `data-no-row-nav` needed. Per-column overrides: `Column.recordUrl(fn)` swaps in a column-specific URL (stamped under `_columnRecordUrls[name]`), and `Column.recordUrl(false)` opts a column out entirely (`meta.recordUrl === false` → renderer skips the link wrap). Empty-state UI auto-distinguishes "filtered but empty" from "no records yet". `formatCell(value, col, row)` switches on `col.columnType` (text/badge/icon/boolean/image), applies built-in `format` spec via `applyColumnFormat`, and reads server-evaluated overrides from `row._formatted[colName]`. **All non-modal action triggers SPA-update:** `dispatchHandlerAction` / `dispatchMethodAction` fetch with `Accept: application/json`, drain `notifications[]` via `useToast()`, then `useNavigate()` to the redirect — no full page reloads. Handler-style actions WITH confirm OR modal → `ActionModalDialog` (same JSON+navigate path with inline 422 errors). Form fields use `defaultValue` (uncontrolled); `FormRenderer` is keyed on `formId` so SPA-nav between forms forces a fresh remount and inputs pick up new values.

  **`ActionModalDialog`** (controlled or uncontrolled). Renders a header (`heading`/`description`/`icon`/`width`), the schema as form fields via `renderFormChild` (so per-field errors render under each input), and a footer with cancel/submit buttons. Controlled mode (`open` + `onOpenChange`) is used by `ActionGroupTrigger` so the dropdown can close before the dialog opens. Confirm-only actions (no schema) reuse the same dialog with no fields rendered.

  **Row-data convention** — server-side eval results land on each row under reserved underscore-prefixed keys: `_visibleActions: string[]` (action names visible for this row), `_disabledActions: string[]` (action names disabled for this row), `_formatted: Record<colName, string>` (custom-formatter output), `_recordUrl?: string` (per-row click target from `Table.recordUrl(fn)`), `_columnRecordUrls?: Record<colName, string>` (per-column overrides from `Column.recordUrl(fn)` — wins over `_recordUrl` for that cell). `renderRowActions` filters `conditional: true` actions through `_visibleActions` and applies the disabled flag from `_disabledActions`. `formatCell` reads `_formatted[name]` first, falling back to the raw value + format spec.

  **Form submit dispatch (Save).** `FormRenderer` intercepts `<form onSubmit>` and fetches with `Accept: application/json`. On success: drains `notifications[]` via `useToast`, then `useNavigate(redirect)` — but skips navigate when `redirect === current URL` so scroll preservation kicks in for the common "save and stay on edit" case. On 422: parses errors into client state and re-renders inline (form values stay because inputs are uncontrolled — DOM holds them; no values mirror needed). On 5xx/network: error toast. Native form-post 303 path preserved as fallback (no `action` URL → falls through). All POST handlers in `routes.ts` (resource create / edit, global edit, custom-page form, delete) honor `Accept: application/json` and return `{ ok, redirect, notifications }` / `{ ok: false, errors }` / `{ ok: false, error }` accordingly.

  **Walkers use structural type checks, not `instanceof`.** `findForms` / `findActions` (in `dispatchForm.ts` / `dispatchAction.ts`) match on `el.getType() === 'form' | 'action'` rather than `instanceof Form / Action`. Vite's SSR module cache loads the package via two paths during a single dev session (rudder route handler vs Vike `+data` hook), giving two different class identities — `instanceof` returned false on the SPA-nav data path and forms/actions went "missing." See `feedback_vite_ssr_module_dup_instanceof.md`.
