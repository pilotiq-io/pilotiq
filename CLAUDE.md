...
- `src/defaultPages.ts` — Owns four exported base classes `ListPage` / `CreatePage` / `EditPage` / `ViewPage`. Each derives slug/label/icon from `getResource()`, calls the resource's `form()`/`table()`/`detail()`, applies model-backed defaults via `applyFormDefaults` / `applyTableDefaults`, and exposes override hooks (`getHeader`, `getActions` on ViewPage). Filament-style usage: `class ListArticles extends ListPage { static override getResource() { return ArticleResource } }`. Factories `defaultListPage(R)` / `defaultPages(R)` etc. return anonymous subclasses bound to R for the no-customization path. View page's default `getActions` returns Edit (link) + Delete (form-post) actions. Sentinel `save`/`loadRecord` only fire when the user hasn't configured them on the Resource form (and `R.model` isn't set).
...
- `src/react/SchemaRenderer.tsx` — Renders resolved schema elements. `TableRenderer` segregates table actions by placement: `header`/`inline` in the top bar, `bulk` in a toolbar that only shows when rows are selected, `row` in a per-row Actions column. Bulk placement triggers a checkbox column (selection state keyed by `row.id` in React state). Handler-style actions (those with `dispatchUrl`) submit a hidden `<form method="POST">` with `ids[]` so the browser handles 303 natively — no fetch round-trip.
