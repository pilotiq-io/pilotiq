...
- `src/pageData.ts` — Per-page-role data builders (`dashboardData`, `resourceIndexData`, `resourceCreateData`, `resourceEditData`, `resourceViewData`, `globalEditData`, `globalViewData`, `customPageData`) plus `dispatchPageData(pageContext)` switchboard. **Navigation tree:** `panelInfo()` is async — it builds a unified `navigation: NavItem[]` (`{ name, label, url, icon?, group?, sort?, badge?, badgeColor?, children? }`) by flattening resources → globals → pages, resolving `navigationParentItem` references into nested children (cycles broken with a console warn), sorting siblings by `navigationSort` then registration order, and resolving every `navigationBadge()` in parallel via `Promise.all` (errors swallow). Replaced the legacy `resources / globals / pages` arrays. **Both** the rudder route GET handlers AND the Vike `+data` hook (auto-generated by the Vite plugin) call these so SSR and SPA-nav serve identical data. Without the dual path, SPA nav lands on a blank page because the rudder handler doesn't run for `*.pageContext.json` fetches. Page renderers read from `(ctx.data ?? ctx.viewProps)`; the data hook short-circuits to viewProps on SSR to avoid double schema-builds. `resolveActiveTab(elements, query, currentPath)` walks for `ListTabs`, picks the active tab from `?tab=` (default → `ListTab.default()` → first), stamps active flag + per-tab URL + parallel-resolved badge counts; called inside `resourceIndexData` before `loadTableRecords` so `dispatchTable.findActiveTab` can splice the tab's `modifyQuery` into the records-handler `TableContext` (see `dispatchTable.ts`).
