...
- `src/react/SchemaRenderer.tsx` — Renders resolved schema elements. `TableRenderer` segregates table actions by placement: `header`/`inline` in the top bar, `bulk` in a toolbar that only shows when rows are selected, `row` rendered inline (each Action a small button next to the cell — Filament-style). Wrap row Actions in `ActionGroup` to opt into a dropdown explicitly. Bulk placement triggers a checkbox column (selection state keyed by `row.id` in React state). Heading / description / striped / emptyState chrome ride on `Table.heading() / .description() / .striped() / .emptyState({heading, description, icon})`. **Row click navigation:** `Table.recordUrl(r => …)` stamps a per-row URL under `_recordUrl`; the row becomes clickable + SPA-navigates via `useNavigate()`. Cells with their own interactive content (bulk checkbox, actions) carry `data-no-row-nav` so clicks there don't double-fire as row navigation. Empty-state UI auto-distinguishes "filtered but empty" from "no records yet". `formatCell(value, col, row)` switches on `col.columnType` (text/badge/icon/boolean/image), applies built-in `format` spec via `applyColumnFormat`, and reads server-evaluated overrides from `row._formatted[colName]`. **All non-modal action triggers SPA-update:** `dispatchHandlerAction` / `dispatchMethodAction` fetch with `Accept: application/json`, drain `notifications[]` via `useToast()`, then `useNavigate()` to the redirect — no full page reloads. Handler-style actions WITH confirm OR modal → `ActionModalDialog` (same JSON+navigate path with inline 422 errors). Form fields use `defaultValue` (uncontrolled); `FormRenderer` is keyed on `formId` so SPA-nav between forms forces a fresh remount and inputs pick up new values.
