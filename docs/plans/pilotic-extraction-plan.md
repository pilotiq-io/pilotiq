...
- [x] **`media` and `workspaces` move to `pilotic/pilotic` in Phase 2.** They become `@pilotic/media` and `@pilotic/workspaces`. Phase 2 scope grows from 2 → 4 packages but eliminates a future cross-repo coupling. Both packages depend on `@rudderjs/panels` today via the `Field` / `PanelPlugin` API; post-extraction they'll depend on `@pilotic/panels` from the same workspace.
- [x] **`Field.ai(['rewrite'])` in free `@pilotic/panels` without pro installed: throw a helpful error at form-build time.** Mirrors the existing `unknown AI action` error pattern. Free users see something like: `Field "title": AI actions require @pilotic-pro/ai. Install it or remove the .ai() call.` Fail loudly, fail fast, fail informatively. Resolution: free `BuiltInAiActionRegistry.get(slug)` returns undefined; the existing throw at `schema/Field.ts:594` catches it with a message updated to point at `@pilotic-pro/ai`.
- [x] **Chat UI vendor:publish: two tags, two publish commands.** `@pilotic/panels` publishes `pilotic-pages`; `@pilotic-pro/ai` publishes `pilotic-ai-pages`. Playground runs both `pnpm rudder vendor:publish --tag=...` commands during sync. The single-tag-multi-source CLI extension is **deferred to a Phase 4 follow-up plan** (small `@rudderjs/cli` improvement) — ship Phase 4 first with the verbose-but-working pattern.
- [x] **All `panels-*` vendor:publish tags rename to `pilotic-*` in Phase 2.** Atomic rename of three tags: `panels-schema` → `pilotic-schema`, `panels-pages` → `pilotic-pages`, plus `panels-translations` → `pilotic-translations` which already shipped in task A of the localization plan.
- [ ] **`PanelAgent` interface vs class split** — explicitly deferred to **Phase 3 design discussion.** Working hypothesis: free `@pilotic/panels` exports `PanelAgent` as a TypeScript `interface` (type contract only); pro `@pilotic-pro/ai` exports `PanelAgent` as a runtime `class` that implements the interface. Same import path, different artifacts depending on whether pro is installed. Requires careful TypeScript handling around declaration merging vs replacement, and the interface needs to be expressive enough to type-check existing `Panel.make().resources([...])` chains in free without pulling AI types. Revisit when drafting the Phase 3 extension hooks design.
...
|---|---|
| `@rudderjs/panels` → `@pilotic/panels` | Same |
| `@rudderjs/panels-lexical` → `@pilotic/lexical` | Same |
| — | `@rudderjs/media` → `@pilotic/media` |
| — | `@rudderjs/workspaces` → `@pilotic/workspaces` |
| Tag renames: `panels-pages`, `panels-translations` (already done) | Add: `panels-schema` → `pilotic-schema` |
