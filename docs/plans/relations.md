...
| 7. Mount on EditPage/ViewPage | ⏳ NOT STARTED | When `R.relations()` returns at least one manager, EditPage/ViewPage wraps the existing schema in a Tabs container: first tab = the form/detail, subsequent tabs = each relation manager's table. Tab labels derive from `RelationManager.label`. |
