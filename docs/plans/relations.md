...
| 8. Authorization fall-through | ✅ DONE 2026-05-01 | New `safeManagerPolicy(M, method, Related, user, parent, child?)` exported from `pageData.ts`. When the manager's predicate is the inherited default (reference-equality check against `RelationManager`'s prototype), defers to the related Resource's matching predicate (parent arg dropped — Resource signatures don't carry parent context). Manager-overridden predicates win unconditionally. Used in all three relation builders + the create/delete POST routes; redundant route-level checks on GET handlers removed (data builder runs the same logic). 7 new tests covering override beats default, default falls through, parent-arg stripping, both-default allow, throw fails closed, integration via relation-list. |
| 9. Reactive integration | ✅ DONE 2026-05-01 | Doc-only for v1. The parent form-post-303 path reloads the page after create/edit/delete, so the manager-tab badge updates incidentally — worst case is a stale badge between batched edits. Limitation captured in `docs/guide/relations.md` ("Reactive integration") with the `Tabs.dependsOn([fields])` follow-up sketched. |
| 10. Tests | ⏳ NOT STARTED | RelationManager registration; `relatedQuery` plumbs through to ModelQuery; routes load+save against the parent; tab mounting; authorization gating; `R.relations()` empty-array short-circuit. (Most of these were covered incidentally by Steps 1-8 — 822 tests.) |
| 11. Playground demo | ✅ DONE 2026-05-01 | `playground-pilotiq`: `User → Posts` (hasMany). New `Post` Prisma model mirrored across both playgrounds (per `feedback_prisma_hoist_shared_schema.md`). New `app/Models/{User,Post}.ts` rudder models with `static relations`, `app/Pilotiq/Users/UserResource.ts`, `app/Pilotiq/Users/relations/PostsManager.ts` (`mutateDataBeforeCreate` stamps `authorId` from `ctx.parentId`), `app/Pilotiq/Posts/PostResource.ts` (so manager row clicks land on `/new-admin/posts/:id`). Verified end-to-end: create user via admin → POST /users/:id/posts/create succeeds twice → GET /users/:id/edit renders Posts tab → GET /users/:id/posts shows the user's posts. |
| 12. Docs | ✅ DONE 2026-05-01 | New `docs/guide/relations.md` (canonical guide — quick example, URLs, reserved tokens, two-layer authorization, lifecycle hooks, reactive limitation, out-of-scope deferrals, custom resource discovery + relatedQuery override). `docs/guide/migrating-from-panels.md` panels `RelationField` → pilotiq `RelationManager` addendum. `CLAUDE.md` updated with `RelationManager.ts` + relation routes. `admin-gap-audit.md` Plan #11 flipped to ✅. README Features list mentions Relations. |
...
**Tests after Step 8:** 822/822 (+7 in safeManagerPolicy + integration).
