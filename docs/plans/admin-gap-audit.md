...
| Feature | Tier | Notes |
|---|---|---|
| `SelectField.createOptionForm()` / `editOptionForm()` / `createOptionUsing()` | 1 | Inline create-record-from-select-modal — heavily-used Filament idiom. Pilotiq's `SelectField` has `options()` + reactive `options(fn)` (Plan #5) but no inline-create UX. Composes cleanly with the existing Action modal infra. Materially closes the "needs to dig into Resource picker" round-trip for create flows. |
