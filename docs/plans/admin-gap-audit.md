...
**Status:** ✅ ESSENTIALLY COMPLETE (re-audited 2026-05-30). Every Tier-1 plan doc and the entire Tier-2 fluent-API fill-out list ship in source — verified against `packages/pilotiq/src`. Only intentionally-deferred Tier-3 items remain open (see "Confirmed-still-deferred" + the list below). The last micro-gap on a shipped surface — `FileUploadField.preserveFilenames()` — shipped 2026-05-30.
...
| Feature | Tier | Notes |
|---|---|---|
| ✅ `RepeatableEntry` DONE | 2 | Shipped 2026-05-06 cont'd⁶ — display-side sibling of `Repeater`. `RepeatableEntry.make(name).schema([Entry…]).columns(n).grid(n).table([cols]).contained(false)`. Reads the array off `ctx.record[name]` and resolves the inner schema once per row with `record` scoped to that row's data, so inner entries (`TextEntry / BadgeEntry / IconEntry / ImageEntry / KeyValueEntry / ColorEntry`) read state via the same `record[childName]` lookup they use elsewhere. Three layout modes (renderer dispatches `table > grid > stack`); v1 takes a fixed integer for `grid()` (the responsive object form is form-side ergonomic — defer until asked). Primitive array elements stash under a reserved `_value` key so a `TextEntry.make('_value')` can target them. Empty / non-array / null falls through to the inherited `default()` placeholder. 14 tests in `RepeatableEntry.test.ts`. |
| ✅ Render hooks DONE | 2 | Shipped — `Pilotiq.renderHook(name, fn, scope?)` with chrome + page-role + global-search + `panels::head.*` mounting all wired. `panels::head.*` (path B — auto-gen `+Head.tsx` mounts `<HeadHooks>` + 4 head-safe primitives `MetaTag/LinkTag/ScriptTag/StyleTag`) shipped 2026-05-07 cont'd⁸ as `1656eaa`. Memory `project_pilotiq_render_hooks.md`. |
| ✅ Filter layout positions DONE | 2 | Shipped 2026-05-06 cont'd⁸ — `Table.filtersLayout('modal' \| 'above-content' \| 'above-content-collapsible' \| 'below-content')`. `'modal'` default keeps the toolbar Filters button + popover. The three inline modes lay every filter widget out as a wrapping strip in the matching slot; collapsible variant adds a toolbar toggle with `localStorage` open/closed persistence per table path (defaults open when any URL filter value is active). Inline modes suppress the active-filter pill row — every widget already shows its current value. Sidebar positions (`BeforeContent` / `AfterContent`) deferred until a consumer asks. |
| ✅ User menu DONE | 2 | Shipped — top-right dropdown driven by `UserMenuItem` + `Pilotiq.userMenuItems()` / `.signOut()`. Rides through `panelInfo()` so SSR + SPA-nav share shape. Memory `project_pilotiq_user_menu.md`. |
| Multi-tenancy | 3 | Large new system — tenants, ownership, switcher. Out of scope for now. |
| ✅ Database notifications DONE | 2 | Shipped — bell affordance + `Notification.sendToDatabase()` + 5 endpoints + `Notification.actions` slot with named-handler registry (closures survive the DB round-trip — beats Filament). Memory `project_pilotiq_database_notifications.md`. |
| ✅ Broadcast notifications DONE | 2 | Shipped — `Notification.broadcast()` + RudderSocket subscription + `private-pilotiq-notifications.*` auth. Memory `project_pilotiq_broadcast_notifications.md`. |
...
| Feature | Tier | Notes |
|---|---|---|
| ✅ `Alert.actions([Action…])` (Callout-parity footer) DONE | 1 | Shipped 2026-05-07 cont'd⁵ — `Alert.make(content).actions([Action…])` lands on `_children` and resolves through the standard schema walker so per-action `.visible()` / `.authorize()` rules fire unchanged. Renderer paints a footer row below the alert body via the same `renderActionLike` dispatch the `Heading.actions(...)` and `EmptyState.footer(...)` slots already use. Bare `Alert.make(...)` with no actions emits no `children` key (zero overhead for existing call sites). 5 tests in `Alert.test.ts` (default content / variant fluent / title / `actions()` resolves children + visibility-context-honored / no children when unset). Tests `2546 → 2552`. `controls([…])` and variadic `controlActions(a, b, …)` ship as **aliases** for the footer `actions()` slot (`schema/Alert.ts:53-63`); v5's distinct right-side in-body icon-button affordance (a separate visual slot) is still deferred — no consumer ask. |
| ✅ Markdown / Html sanitization DONE | 1-2 | Shipped 2026-05-07 cont'd⁷ — both display primes sanitize their HTML server-side via `sanitize-html` (new dep) against a prose-friendly default allowlist (`<p>`, headings, lists, code, tables, links, images), matching Filament v5's default-secure stance. `<script>`, `<iframe>`, `javascript:` URLs, inline `onclick=` handlers and `style="..."` overrides are stripped before the wire shape ships. Three escape hatches: `.allowRaw()` (clearest at call site) / `.sanitize(false)` (explicit form) / `.sanitize({ allowedTags, allowedAttributes, allowedSchemes })` widening for trusted CMS embeds. Allowlist + helper live at `src/schema/sanitize.ts` (`DEFAULT_SANITIZE_CONFIG` exported next to `sanitizeHtml(html, config?)`). 9 new tests in `primes.test.ts` (sanitize default for both primes / `<script>` strip / `javascript:` strip / inline-handler strip / `allowRaw()` opt-out / `sanitize(false)` explicit form / custom config widens for `<iframe>`). Tests `2557 → 2566`. Companion `MarkdownField` / `RichTextField` editor surfaces stay admin-trusted (no client-side sanitizer in v1) — sanitize on the way out via these display primes. |
| ✅ `SingularResource` (single-record settings) DONE | 2 | Shipped 2026-05-07 cont'd⁶ — widened `Global.model` from a vestigial `string` identifier to a real `ModelLike`. When set, `defaultGlobalEditPage` (and `defaultGlobalViewPage`) auto-wire `Form.loadRecord` (via new `loadSingularRecord(M, opts?)` helper in `orm/modelDefaults.ts` — `paginate(1, 1)` over `M.query()` returns the first row or `null`) and `Form.save` (via existing `modelSave(M)` — `M.update(pk, data)` when `ctx.record` exists, `M.create(data)` on first POST so the singleton auto-creates on first save). Hand-wired `Form.loadRecord` / `Form.save` always win — auto-wire only fills empty slots. New `static findSingular?(q): ModelQuery` opt-in scope override for fixed-id (`q.where('id', '=', 1)`) or slug-style (`q.where('key', '=', 'site')`) lookups. 5 new tests in `Global.test.ts` (no-model passthrough / auto-wire happy path / first-save-creates-then-updates / hand-wired wins / `findSingular` query shaping incl. miss → null). Tests `2552 → 2557`. The reference admin's "singular resource" pattern thus lands on the structurally-close `Global` class rather than a new `SingularResource` sibling — keeps the "Resource = many records, Global = one record (settings)" two-class story intact. Docs: `docs/packages/pilotiq/globals.md` minimal example flipped to show `static model =` shortcut + new "Singular-record auto-wiring" section + prior "loadRecord for singletons" renamed to "Hand-wired loadRecord (without `static model`)". |
...
| ✅ Custom-extension scaffolder docs DONE | 3 | Shipped 2026-05-07 cont'd⁹ as `0c48eed` — `docs/guide/extending-pilotiq.md` consolidates Field / Column / Entry / Widget extension points + the two-registry split (`registerWidgetComponents` for the `View` escape hatch vs `registerWidgetRenderer` for whole new widget element types). Memory `project_pilotiq_extension_scaffolder_docs.md`. |
...
| ✅ `Column.beforeStateUpdated()` / `afterStateUpdated()` DONE | 1 | Shipped 2026-05-09 cont'd². Editable-cell PATCH lifecycle hooks. `beforeStateUpdated((value, { record, user }) => …)` runs after validators pass and before the DB write; `afterStateUpdated` fires only on a confirmed update. Throwing → 422 with the message stamped under the reserved `_cell` error key. Live on `Column` base (gated by `isEditable()`) so all three editable subclasses (`TextInputColumn / ToggleColumn / SelectColumn`) inherit. EditableCell renderer reads `errors._cell` alongside `errors.value` for the failure toast. 8 new tests across `Column.test.ts` and `routes.test.ts`. |
| ✅ `Wizard.submitAction()` / `nextAction()` / `previousAction()` DONE | 2 | Shipped — nav-button customization on `Wizard` (`schema/Wizard.ts:185-198`). |
| ✅ `Wizard.persistStepInQueryString()` DONE | 2 | Shipped — active step synced to a URL param (`schema/Wizard.ts:170-172`), not localStorage-only. |
| ✅ `Repeater.afterCreate()` / `afterUpdate()` / `afterDelete()` (relationship-mode) DONE | 2 | Shipped — the after-* trio alongside the mutate-* siblings (`fields/RepeaterField.ts:624-661`). |
| ✅ `SelectColumn.options(fn)` reactive resolver DONE | 2 | Shipped — `options()` accepts a per-row callback receiving the record (`columns/SelectColumn.ts:67-76`). |
| ✅ `Column.toggleable()` DONE | 2 | Shipped — user-toggleable column visibility w/ `initiallyHidden` (`Column.ts:300-314`). |
...
**Newly surfaced gaps** — none earth-shaking; mostly fill-out-the-fluent-API work on existing surfaces:

| Feature | Tier | Notes |
|---|---|---|
| ✅ `SelectField.createOptionForm()` / `createOptionUsing()` DONE | 1 | Shipped (`select-create-option.md`). Inline create-record-from-select-modal — "+" trigger → Dialog → `POST …/_form/:formId/create-option/:fieldName` (globals / pages / resource create + edit) → append + select. `createOptionAuthorize` gate, `formCreateOptionData` + `tagSelectCreateOptionUrls` walker, `SelectFieldInput.tsx` client flow. `editOptionForm()` + relation-scope routes deferred as planned. |
| ✅ Action modal chrome extras DONE | 1-2 | All shipped. `closeModalByClickingAway(false)` / `closeModalByEscaping(false)` / `stickyModalHeader()` / `stickyModalFooter()` / `modalAutofocus()` / `modalCancelActionLabel()` / `modalSubmitActionLabel()` / `modalAlignment()` / `modalIconColor()` / `modalCloseButton()` shipped 2026-05-07. Carved-off `modalContentFooter([Element…])` shipped 2026-05-08 — auxiliary Elements between body and Cancel/Submit footer; resolves through the standard walker so inner Action `.visible() / .disabled()` rules evaluate the same as anywhere else. |
| ✅ `TextInputField` rich affordances DONE | 2 | Shipped 2026-05-08. `password()` / `revealable()` / `copyable(message?)` / `mask(pattern)` / `stripCharacters(chars)` / `datalist([…])` / `inputMode()` / `autocapitalize()` / `prefixAction(Action)` / `suffixAction(Action)` all on `TextField`. Mask alphabet is `9` digit / `a` alpha / `*` any / literals passthrough. `stripCharacters` runs server-side (in `coerceFormValues`) AND client-side. `prefix/suffixAction` resolve through the standard schema walker so inner Action `.visible() / .disabled()` rules evaluate the same way as anywhere else. New `useTextInputControls(el, name, renderAction)` hook owns reveal/copy/mask state in a `<TextFieldShell>` component to keep `renderField` hook-free. `FieldShell` widened with `before` / `after` ReactNode slots. **`TextField.trim(v=true)` (2026-05-09 cont'd):** server-side leading/trailing whitespace strip mirroring Laravel's `TrimStrings` middleware. Runs BEFORE `stripCharacters` in `coerceFormValues` so a value like `'  (415) 555-1212  '` first trims, then has mask literals removed. Composes with the existing rich-affordance setters; emit-only-when-set on the meta. 6 new tests in `TextField.test.ts`. |
| ✅ `TextareaField.autosize()` / `cols()` / `disableGrammarly()` DONE | 2 | Shipped — all three wired (`fields/TextareaField.ts:27,38,47`). |
| ✅ `TextColumn` rich-display methods DONE | 2 | Shipped on `Column` base (`Column.ts:401-468`) — `words(n)` / `characters(n)` / `listWithLineBreaks()` / `bulleted()` / `copyMessage()` / `markdown()` / `html()`. |
| ✅ `FileUploadField.imageEditor()` family DONE | 2 | Shipped (`fields/FileUploadField.ts`) — `imageEditor()` + `imageEditorAspectRatioOptions()` + `circleCropper()` + `downloadable()` / `openable()` / `reorderable()` / `panelLayout()` / `avatar()` / `appendFiles()`. **`preserveFilenames()` shipped 2026-05-30** — field flag → `preserveFilenames` meta → FormData → `handleUploadRequest` → `UploadRequest.preserveFilenames` → `localUpload` keeps the sanitized original name (random-id fallback on empty; same-name overwrites). |
| ✅ `Repeater.expandAllAction()` / `collapseAllAction()` / `expandAction()` DONE | 2 | Shipped (`fields/RepeaterField.ts:707,719-736`). `fixIndistinctState()` not separately verified. |
| ✅ `Alert.controls()` / `controlActions()` DONE | 2 | Shipped (`schema/Alert.ts:53-63`) as Alert methods rather than a new `Callout` class. `footerActionsAlignment()` / `dismissible()` / `iconSize()` not separately verified. |
| ✅ `Section.secondary()` / `afterHeader()` DONE | 2 | Shipped (`schema/Section.ts:98,124-127`). |

**Confirmed-still-deferred** (no urgency change): `CheckboxColumn` (semantic-equivalent ToggleColumn already shipped), Multi-tenancy, MFA, Plugin API surface (standalone-plugins / configurable-resources-and-pages / asset registration), CSS hooks (`fi-*` convention), Testing utilities, `configureUsing()` global defaults, resource depth-3+ URL nesting (depth-2 audit declared no-op).
...
| ✅ Default-query override (global scopes) DONE | 1 | Shipped 2026-05-04 cont'd¹⁷ — `Resource.query(ctx?: QueryContext): ModelQuery` + `findRecord(R, id, ctx)` helper. Default returns `this.model.query()`; override to install tenant scopes, default ordering, eager-load defaults, etc. Threaded through `modelTableRecords` (list pages), `modelLoadRecord` (form fill), `findRecord` (record-aware policy lookups + view/edit/action loads in `routes.ts` + `pageData.ts` — ~14 sites), `search.ts` (global search query starts from `R.query(ctx)` so scopes apply to Cmd+K hits too). Soft-delete restore / force-delete deliberately bypass — those build their own `query().withTrashed()` chain so a scope that hides trashed rows doesn't hide records the operator is trying to recover. **Counts:** `@pilotiq/pilotiq` 1997 (was 1985, +12). |
...
| ✅ Custom resource pages (sibling of List/Create/Edit/View) DONE | 1 | Shipped 2026-05-04 cont'd¹⁸ — pure docs row. Pattern (already-shipped surfaces): register a regular `Page` subclass via `panel.pages([P])`, set `static navigationParentItem = 'PostResource'` (Plan #9 — by JS class name) so the nav tree nests it under the resource sidebar item. URL is a panel-level sibling (`/admin/post-analytics`), not a child of the resource (`/admin/posts/analytics`); true URL nesting stays Tier-3. New "Custom resource pages" section in `docs/packages/pilotiq/pages.md` covers nav nesting, authorization gate, active-state highlighting, and the URL trade-off. |
...
| ✅ `getFormActions` on EditPage (delete/view buttons in header) DONE | 1 | Shipped 2026-05-04 cont'd¹⁸ — small surface widening + docs. `EditPage.getFormActions(R, basePath, recordId?)` widened from `(R)` so overrides can call `Action.delete / .view(R, basePath, recordId)` factories without hardcoding the panel mount path. `CreatePage.getFormActions(R, basePath)` widened in parallel for Cancel-link parity. `EditPage.schema` / `CreatePage.schema` now thread `ctx?: SchemaContext` to read `basePath` + `recordId`. Default behavior identical (existing `(R)` overrides still compile under TS's wider-override rule). Pure additive on the consumer side. **Counts:** `@pilotiq/pilotiq` 2000 (was 1997, +3). Docs: new "EditPage header actions" subsection in `docs/packages/pilotiq/pages.md`. |
...
| ✅ `columnSpan` / `columnStart` / `columnOrder` (positional control inside Grid) DONE | 1 | All three shipped on the `Element` base (`schema/Element.ts:178-185` setters → `meta._layout` → `schemaRenderer/helpers.tsx` maps to `col-span-*` / `col-start-*` / `order-*`, clamped 1-12). `columnSpan` landed with **Plan #8**; `columnStart` / `columnOrder` followed (re-audit 2026-05-30 corrected a stale "deferred" note here — they were already live). |
...
| ✅ `Repeater` (array of sub-schemas) DONE | 3 | Shipped 2026-05-01 (Plan #14) + extensive Tier-1/2 polish + relationship-backed rows: `hasMany` 2026-05-03, `morphMany / morphOne` 2026-05-05, **M2M family (`belongsToMany / morphToMany / morphedByMany`) 2026-05-05** (`repeater-relationship-m2m.md`) — submit creates the related child + attaches via `parent[rel]().attach()`; row-removal detaches only (no `M.delete` — child may be linked to other parents); `orderColumn()` rejected under M2M (ORM has no `orderByPivot`). See `repeater-field.md` + `repeater-relationship.md` + `repeater-relationship-m2m.md` + `docs/guide/repeater.md`. |
| ✅ `Builder` (heterogeneous array) DONE | 2 | Shipped 2026-05-02 — `Block.make(name).schema(…).maxItems(n)` + `Builder.make(name).blocks([…])`, `{type, data}` storage envelope, per-block `maxItems`, server-side resolve / coerce / validate / partial-resolve mirroring Repeater. Relationship-backed rows shipped 2026-05-05 (`Builder.relationship(name)` — stores rows as child records carrying `type` discriminator + JSON `data` payload; `hasMany` + `morphMany / morphOne` only; **M2M family deferred** — the `{type, data}` envelope doesn't compose with pivot semantics; `repeater-relationship-m2m.md` raises a clear error pointing at `Repeater.relationship`). Demo at `/new-admin/builder-demo`. See `builder-field.md` + `builder-relationship.md` + `docs/guide/builder.md`. |
