...
| Action modal chrome extras | 1-2 | `closeModalByClickingAway(false)` / `closeModalByEscaping(false)` / `stickyModalHeader()` / `stickyModalFooter()` / `modalAutofocus()` / `modalCancelActionLabel()` / `modalSubmitActionLabel()` / `modalAlignment('start' \| 'center')` / `modalIconColor()` / `modalCloseButton(false)` / `modalContentFooter()`. Cheap pure-chrome pass-through fields on `Action.ts`. |
