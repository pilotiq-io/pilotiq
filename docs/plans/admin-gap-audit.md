...
| `TextareaField.autosize()` / `cols()` / `disableGrammarly()` | 2 | Currently only `rows()` is wired. `autosize()` is the most frequently-asked. |
| `TextColumn` rich-display methods | 2 | `words(n)` / `characters(n)` (truncation by units other than `limit(chars)`) / `listWithLineBreaks()` / `bulleted()` / `copyMessage(s)` + `copyMessageDuration(ms)` / `markdown()` + `html()` cell-level formatters (route through the existing `sanitize-html` allowlist from the Markdown / Html primes — straightforward reuse). |
| `FileUploadField.imageEditor()` family | 2 | In-browser cropper (`imageEditor()` + `imageEditorAspectRatioOptions()` + `circleCropper()` + `automaticallyCropImagesToAspectRatio()` + `automaticallyResize…`) plus `downloadable() / openable() / reorderable() / panelLayout('grid' \| 'integrated') / avatar() / preserveFilenames() / appendFiles()`. Useful for any media-heavy panel. |
| `Repeater.expandAllAction()` / `collapseAllAction()` / `expandAction()` | 2 | Top-level expand-all/collapse-all triggers (Pilotiq has per-row `collapseAction()` only). Useful pair when `accordion()` or `collapsible()` is set. Adjacent: `fixIndistinctState()` (auto-resolve duplicates triggered by `distinct()`). |
| `Callout` controls slot vs `Alert` | 2 | Filament v5's `Callout` is a strict superset of `Alert`. Beyond `actions()` (just shipped), it carries `controls()` (top-right slot) / `controlActions()` (icon-button actions in top-right) / `footerActionsAlignment('start' \| 'center' \| 'end' \| 'between')` / `dismissible()` / `iconColor()` + `iconSize()`. Closest analogue exists; could land as `Alert.controls([…])` + `Alert.controlActions([…])` rather than a new class. |
| `Section.secondary()` / `afterHeader()` | 2 | `.secondary()` (less-contrasting bg variant) + `.afterHeader([Action…])` (right-side header slot). Pilotiq has `dense / compact / aside / collapsible / persistCollapsed / icon / badge` but neither of those two. Cheap. |
