...
**Status:** ✅ ESSENTIALLY COMPLETE (re-audited 2026-05-30). Every Tier-1 plan doc and the entire Tier-2 fluent-API fill-out list ship in source — verified against `packages/pilotiq/src`. Only intentionally-deferred Tier-3 items remain open (see "Confirmed-still-deferred" + the list below). The sole micro-gap on a shipped surface: `FileUploadField.preserveFilenames()`.
...
| ✅ `FileUploadField.imageEditor()` family DONE | 2 | Shipped (`fields/FileUploadField.ts:86-114`) — `imageEditor()` + `imageEditorAspectRatioOptions()` + `circleCropper()` + `downloadable()` / `openable()` / `reorderable()` / `panelLayout()` / `avatar()` / `appendFiles()`. **Only `preserveFilenames()` still missing** — minor, ship if asked. |
