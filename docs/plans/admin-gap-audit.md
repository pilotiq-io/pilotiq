...
| `TextInputField` rich affordances | 2 | `mask()` (Alpine-style input mask) / `datalist([…])` / `revealable()` (password toggle) / `copyable()` / `prefixAction()` + `suffixAction()` (clickable Action in input shell — distinct from current passive `prefix/suffix`) / `stripCharacters()` / `inputMode()` / `autocapitalize()`. `password()->revealable()` is a common ask. |
