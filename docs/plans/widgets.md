...
| 6. `Stat` value object + `StatsOverview` element | ⏳ NOT STARTED | `Stat.make(label, value?)` fluent setters: `value / description / descriptionIcon(icon, position?) / icon / color / chart([n…]) / url / openUrlInNewTab / extraAttributes`. `StatsOverview` Element returns `Stat[]` from `getStats(ctx)`; `columns(n)` lays out the row. Sparkline = inline SVG, no chart-lib dep. |
| 7. `@pilotiq/recharts` adapter package | ⏳ NOT STARTED | New workspace package mirroring `packages/codemirror/`. Exports `Chart` Element + `registerChartRenderer()` boot helper (parallel to `registerCodeEditor`). Boot guard throws if a `Chart` element ships in schema without the renderer registered. |
| 8. `Chart` element surface | ⏳ NOT STARTED | `Chart.make(id).label().type(t).getData(fn).color(c).maxHeight(px).options(raw).filters({key: label}).poll(s)`. 8 chart types: `line/bar/pie/doughnut/radar/polar/scatter/bubble` (v1 ships line/bar/pie/doughnut renderers; rest follow as Recharts mappings land). Data shape `{ labels: string[], datasets: Array<{ label, data: number[], color? }> }` (Chart.js shape — Recharts adapter normalizes). Per-chart filter dropdown reads/writes `ctx.filter`. |
| 9. `Table` widget element | ⏳ NOT STARTED | `Table.make(id).resource(R).query(q => q.limit(5)).columns([...])`. Slim mode of `TableRenderer` — no filters, no bulk, no pagination, optional `.viewAllUrl()`. |
| 10. `Resource.headerSchema() / footerSchema()` | ⏳ NOT STARTED | New hooks; default `[]`. Wired into `resourceIndexData`. Renders schema arrays above/below the list table. Same hooks gated by `R.canAccess(user)`. |
| 11. `Page.schema()` widget passthrough | ⏳ NOT STARTED | Already accepts schema; verify Stat/Chart/Table/View resolve correctly inside layout primitives (`Group / Card / Section / Split`). May need a single dispatch tweak in `resolveSchema` to cover the new server-data hook. |
| 12. Tests | ⏳ NOT STARTED | Server-data resolution parallel + ordering; lazy skeleton wire; polling endpoint contract + filter pass-through; canView gate; Stat fluent surface (incl. url/extraAttributes/descriptionIcon); StatsOverview render; Chart filter dropdown; Table widget slim render; Page dashboard sugar; Resource header/footer schema. Target +70-90 tests. |
| 13. Playground demo | ⏳ NOT STARTED | `playground-pilotiq`: `MyDashboard` Page with `UsersStats`, `PostsChart` (per-day line, 'today/week/month' filter), `RecentPosts` (TableWidget on `Post`), and a `View` widget showing how to drop down to custom React. Mounted via `panel.dashboard(MyDashboard)`. |
| 14. Docs | ⏳ NOT STARTED | `docs/guide/widgets.md` — quick example, builtins reference, lazy loading, polling, per-chart filter, recharts install pointer, custom widget recipe (extending Element with `getData`). README features list. `migrating-from-panels.md` addendum (panels has no equivalent). `admin-gap-audit.md` Plan #15 tick. |

**Tests at start:** 1470/1470. Build clean.
**After Phase A (2026-05-03):** 1523/1523 (+53). Build + typecheck clean.
**Target at completion:** ~1545-1560 (+75-90).
