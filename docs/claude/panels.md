...
- PanelAgent `edit_text` is a `.server()` tool: collab fields -> `Live.editText/editBlock` with AI cursor; non-collab -> string ops + `Live.updateMap`.
