...
`@pilotiq/panels` turns your ORM models into fully functional admin interfaces with zero boilerplate. You define what data looks like; panels handles the rest.
...
import { Resource, TextField, EmailField, SelectField, DateField } from '@pilotiq/panels'
...
# After upgrading @pilotiq/panels — update to latest UI
...
> **Tip:** Alias the import if it conflicts with a schema element name (e.g. `Table` from `@pilotiq/panels`).
...
The full list of override keys is the `PanelI18n` type in `@pilotiq/panels` — open `node_modules/@pilotiq/panels/dist/i18n/en.d.ts` for an authoritative reference, or browse the [bundled `en.ts` source on GitHub](https://github.com/rudderjs/rudder/blob/main/packages/panels/src/i18n/en.ts).
...
| [Editor](./editor) | Rich-text editor registry, `@pilotiq/lexical` |
