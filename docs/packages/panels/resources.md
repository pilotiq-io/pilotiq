...
Resources are the core building block of `@pilotiq/panels`. Each resource maps to an ORM model and defines the fields, filters, and actions for that model's CRUD interface.
...
export class UserResource extends Resource {
  ...
  static rememberTable = true               // persist filters, sort, search, page & selection in sessionStorage
...
### Form State Persistence (`static draftRecovery = true`)
...
```ts
import { Resource, Stats, Stat, Chart } from '@pilotiq/panels'
...
```tsx
import { WidgetRenderer } from '@pilotiq/panels/client'
