...
`@pilotiq/panels` uses a pluggable editor registry for rich-text fields. By default, rich content fields render as plain `<textarea>` elements. Install `@pilotiq/lexical` to upgrade them to a full Lexical rich-text editor.
...
```ts
import { RichContentField, Block, TextField, SelectField } from '@pilotiq/panels'
...
```ts
import { Block, TextField, NumberField } from '@pilotiq/panels'
...
When `@pilotiq/lexical` is installed, the editor includes:
...
`@pilotiq/lexical` ships its own styled components. To ensure Tailwind picks up the classes used by the editor, add a `@source` directive to your CSS:
...
This keeps `@pilotiq/panels` lightweight -- the Lexical dependency (~200KB) is only pulled in when you need rich-text editing.
...
```bash
pnpm rudder vendor:publish --tag=pilotiq-schema
```
