...
`Table` is a standalone schema element for `@pilotiq/panels`. Use it on panel landing pages (`Panel.schema()`), resource show pages (`Resource.widgets()`), or anywhere a schema element is accepted. It is distinct from the Resource CRUD list — `Table` is for display-only or lightweight editing scenarios where you want full control over the data source and columns.
...
```ts
import { SelectField, ToggleField, TextareaField, ColorField, TagsField } from '@pilotiq/panels'
