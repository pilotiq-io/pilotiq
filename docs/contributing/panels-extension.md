...
A panels extension adds new capabilities to `@pilotiq/panels` without modifying it — new field types, editor integrations, custom renderers. `@pilotiq/lexical` is the canonical example.

---

## How panels extensions work

`@pilotiq/panels` ships with a **registry system** for pluggable components:
...
This is the **critical rule**: `@pilotiq/panels` must never import `@pilotiq/panels-*`. The dependency only flows one way.
...
```json
{
  ...
  "devDependencies": {
    "@pilotiq/panels": "workspace:*",
    "react": "^19"
  }
}
```

`@pilotiq/panels` is a **peer dependency** (user must have it) and a **devDependency** (for local development). It is never in `dependencies`.
...
```ts
// src/index.ts
import { editorRegistry } from '@pilotiq/panels'
import type { EditorProps } from '@pilotiq/panels'
...
import { myExtProvider } from '@pilotiq/panels-myext'
...
**Never** statically import your extension from inside `@pilotiq/panels`. Instead, `+Layout.tsx` (the panels layout, published to the app) loads extensions dynamically:
...
```css
/* src/index.css */
@source "../node_modules/@pilotiq/panels-myext/dist";
```
...
const SSR_EXTERNALS = [
  // ... existing entries
  '@pilotiq/panels-myext-heavy-dep',
]
```
...
Study `@pilotiq/lexical` as the complete reference:

| File | Purpose |
|---|---|
| `packages/lexical/src/index.ts` | Exports `registerLexical()` |
| `packages/lexical/src/LexicalEditor.tsx` | The heavy React component |
| `packages/lexical/pages/` | Published `CollaborativePlainText.tsx` etc. |
