...
| `.grid(n \| { default?, sm?, md?, lg?, xl?, '2xl'? })` | Lay the *rows themselves* in an n-column grid. Different from `.columns(n)`, which grids the inner schema *inside* a row. **Scalar form** `(grid(2))` is fixed at every viewport. **Responsive form** `(grid({ default: 1, md: 2, xl: 3 }))` keys column counts off the standard Tailwind breakpoints (sm 640px / md 768px / lg 1024px / xl 1280px / 2xl 1536px); the renderer emits a scoped `<style>` block per field. n < 2 (or all-empty object) is the off sentinel. The drag-drop indicator is suppressed in grid mode (it reads wrong across cells); ↑ / ↓ buttons + DnD itself still work. |
