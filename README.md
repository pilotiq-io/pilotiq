...
- **Filters** — `SelectFilter` / `BooleanFilter` / `TernaryFilter` (yes/no/blank — distinguishes NULL from "any") / `DateRangeFilter` (`from..to`-encoded URL value, with `parseDateRangeValue` helper). More kinds extend the `Filter` base. Custom `query(fn)` hook for non-default ORM behavior.
